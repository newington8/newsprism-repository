...
한국어로 핵심 내용을 아래 형식으로 상세하게 요약해 주세요.
전체 분량은 약 6,000자(byte) 이상으로 충분히 작성하세요.

[★ 필수 규칙 ★]
- 모든 수치·통계·퍼센트·달러금액·베이시스포인트는 원문 영어 그대로 인용 (예: `$24.3B`, `+12% YoY`, `65 cents EPS`)
- 경영진 발언·중요 문장은 반드시 영어 원문을 `> "..."` 형식으로 인용 후 한국어 번역 병기
- 기업명·제품명·부서명·지역명은 영어 원문 그대로 사용

[출력 형식]
1. **실적 요약** (매출·순이익·EPS·마진·YoY 성장률 등 모든 수치 상세 기술)

2. **세그먼트별 실적** (사업부문·지역별 매출 breakdown, 수치 원문 인용)

3. **경영진 핵심 발언** (CEO·CFO 중요 발언 5~7가지, 각 발언마다 영어 원문 인용)
   > "영어 원문" → 한국어 번역

4. **가이던스** (다음 분기·연간 전망, 수치 원문 그대로 상세 기술)

5. **주요 리스크 & 도전과제** (경영진 언급 위험 요소, 영어 원문 인용 포함)

6. **Q&A 하이라이트** (애널리스트 질문 중 핵심 3~5가지 및 경영진 답변 요약)

7. **투자 시사점** (종합적인 투자 관점, 긍정·부정 요인 균형 있게 기술)
